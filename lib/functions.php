 ...
 */
    ...
    function dump_debug($input, $caption = "",$collapse=TRUE) {
        if($caption){
            echo "<h2>" . $caption . "</h2>";
        }
    ...
    function getPhpHidden($file,$json = NULL){
        $content = '';
        if(file_exists($file)){
    ...
    function savePhpHidden($file,$data,$to_json = NULL){
    ...
    function namedCharacterToNumeric($string){
    ...
    function template( $names, $args ){
        ...
        foreach ( $names as $name ) {
            $file = DOC_ROOT . 'templates/' . $name . '.tpl';
    ...
    function unifiedPath($path){
        return str_replace('\\','/',str_replace('//','/',$path));
    }
    function path($file){
        $file = unifiedPath($file);
        $root = unifiedPath(DOC_ROOT);
    ...
    function fnc() {
        ...
        if (!function_exists($func)){
            $path = DOC_ROOT . 'functions/';
            include_once($path.$func.'.php');
        }
        return call_user_func_array($func, $args);
    }
    function escapeJavaScriptText($string)
    {
        return str_replace("\n", '\n', str_replace('"', '\"', addcslashes(str_replace("\r", '', str_replace('/', '\/', (string)$string)), "\0..\37'\\")));
    }
    function reversePathinfo($path_parts){
        ...
        $file_arg = array_filter($file_arg);
        $basename = implode(".", $file_arg);
        $path_arg = array(
            $dirname,
            $basename
        );
        $path_arg = array_filter($path_arg);
        return unifiedPath(implode("/", $path_arg));
    }
    
    function sluggedFilename($file){
        ...
        return reversePathinfo($path_parts);
    }
?>